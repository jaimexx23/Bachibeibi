...
function extractStudentCode(rawText) {
  ...
  try {
    ...
    // If parsing URL fails, continue with text-based extraction.
  }
...
async function sendCheckin(code, source = "qr") {
  ...
  const now = Date.now();
  if (normalizedCode === lastCode && now - lastScanTime < 4000) {
    return;
  }
  ...
  if (response.ok) {
    resultBox.innerHTML = `<div class="ok">${data.message}: <strong>${data.student}</strong> (${data.classroom}) - ${data.time}</div>`;
  } else {
    resultBox.innerHTML = `<div class="error">${data.message}${data.student ? `: <strong>${data.student}</strong>` : ""}</div>`;
  }
}
